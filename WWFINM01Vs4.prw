...
-----------------------------------------------------------------------------*/
    ...
    if bI 
        MsgAlert("Campo Cod Serviço em BRANCO, nos codigos de fornecedores a seguir: "+ cAux, "Campo Codigo de Servico")
        RETURN NIL
    endif
...
-----------------------------------------------------------------------------*/
    cCentCod:= IIF (QRY-> A2_COD_MUN != "23404", "6", "5")
    cDezCod:=  IIF (Alltrim(StrTran( QRY-> A2_CSERV , "." , "" ))!= "702", "1", "2")
    cUniCod:=  IIF (QRY->FT_VALICM == 0, "1", "2")

    cCFPS:=(cCentCod + cDezCod + cUniCod )
     cCFPS:=IIF(cCFPS=="612","613",cCFPS)
...
Static Function REGISTRO()
    ...
    aAdd( aHeader, { 'MesReferencia          ',  018,     019,     2,   'N',    0,    'StrZero(month(stod(QRY->FT_ENTRADA)),2) '})  //Mes da declaracao
    aAdd( aHeader, { 'AnoReferencia          ',  020,     023,     4,   'N',    0,    'year(stod(QRY->FT_ENTRADA))'})  //Ano da declaracao
    ...
    aAdd( aDetail, { 'CodAtividade           ',  244,     249,     6,   'N',   0,    'STRTRAN(ALLTRIM(QRY-> A2_CSERV),".","")' } )  //CodServiço do prestador
